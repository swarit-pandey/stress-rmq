...
func NewConnectionManager(config Config) (*ConnectionManager, error) {
	...
	if config.EnableTLS {
		...
		conn, err = amqp.DialConfig(config.RabbitMQURL, amqp.Config{
...
// NewConsumer creates a new consumer with a channel from the connection manager
func NewConsumer(id int, config Config, connManager *ConnectionManager, messagesChan chan MessageData) (*Consumer, error) {
...
func main() {
	...
	connectionName := flag.String("conn-name", "rabbitmq-consumer", "RabbitMQ connection name")
	...
	for i := 0; i < config.NumberOfConsumers; i++ {
		consumer, err := NewConsumer(i, config, connManager, messagesChan)
		if err != nil {
